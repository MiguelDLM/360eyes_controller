###
# IPC365/360Eye PTZ Camera Integration
# Adapted for proprietary TCP protocol
####
import socket
import time
import logging
...
class CameraType(Enum):
    ...
    YCC365 = "YCC365"
    Y05 = "Y05"
    IPC365 = "IPC365"  # Added support for IPC365
...
YOOSEE_SERVICE_PATH = ":23456/onvif/deviceio_service"

# IPC365 configuration
IPC365_DEFAULT_PORT = 23456
IPC365_TCP_TIMEOUT = 5

# IPC365 PTZ Commands (hexadecimal) - COMANDOS VERIFICADOS QUE FUNCIONAN
IPC365_COMMANDS = {
...
YOOSEE_NONCE_STR = str(random.randint(100000, 999999))
...
def setup(hass, config):
    def stop(host, cameraType):
        # IPC365 support
        if cameraType == CameraType.IPC365.name:
            return send_ipc365_command(host, 'stop')
        ...
        # Solo verificar status code si existe la variable r
        if 'r' in locals() and r.status_code != requests.codes.OK:
    ...
    def move(call, move_time, x_coord=0.0, y_coord=0.0, use_stop=True):
        ...
        if cameraType == CameraType.IPC365.name:
            ...
            if direction:
                # Enviar comando con duración automática si use_stop es True
                duration = move_time if use_stop else 0
                success = send_ipc365_command(host, direction, duration=duration)
                
                # Si no usamos auto-stop, hacer pausa manual y stop manual
        ...
        # Solo hacer sleep para otros tipos de cámara (no IPC365)
        if cameraType != CameraType.IPC365.name:
            time.sleep(move_time)

        if use_stop and cameraType != CameraType.YOOSEE.name and cameraType != CameraType.IPC365.name:
    ...
    def move_to_preset(call):
        ...
        elif cameraType == CameraType.IPC365.name:
            send_ipc365_command(host, preset_token)

    def send_ipc365_command(host, command, port=IPC365_DEFAULT_PORT, duration=None):
        """Send PTZ command to IPC365 camera via TCP - Funciona como el script exitoso"""
        try:
            ...
            if command in ['right', 'left', 'up', 'down'] and duration and duration > 0:
                time.sleep(duration)
                # Enviar comando stop usando la misma conexión
